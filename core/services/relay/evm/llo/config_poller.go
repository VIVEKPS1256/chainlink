...
type LLOConfigPollerService interface {
	services.Service
...
type ConfigCache interface {
	// TODO: Is it helpful to use this as a real cache and lookup here in the tracker?
...
func NewConfigPoller(lggr logger.Logger, lp LogPoller, cc ConfigCache, addr common.Address, donID uint32, instanceType InstanceType, fromBlock uint64) LLOConfigPollerService {
...
func (cp *configPoller) LatestConfigDetails(ctx context.Context) (changedInBlock uint64, configDigest ocrtypes.ConfigDigest, err error) {
	latestConfig, log, err := cp.latestConfig(ctx, int64(cp.fromBlock), math.MaxInt64)
...
func (cp *configPoller) latestConfig(ctx context.Context, fromBlock, toBlock int64) (latestConfig FullConfigFromLog, latestLog logpoller.Log, err error) {
	...
	// Cleanup:
	//     []query.Expression{
	//     NewAddressFilter(addr),
	//     {BoolExpression: query.BoolExpression{
	//        BoolOperator: query.OR,
	//        Expressions: query.Expression{
	//            NewEventSigFilter(event1),
	//            NewEventSigFilter(event2),
	//            NewEventSigFilter(event3),
	//        }
	//    }}
	// }
	// (edited)
	...
	//   Today at 2:06 PM
	// @Ilija
	// : curious if this is similar for ChainReader? Do we have any more convenient way of specifying a filter that matches a list of events? I would imagine it’s a fairly common pattern among product plugins… didn’t realize this was missing from the LogPoller DSL until looking through the code just now. (edited)

	// Domino Valdano
	//   Today at 2:12 PM
	// Should be trivial to add a helper function for it, if it’s not already there somewhere
	// New
	// Saved for later
	...
	//   2 minutes ago
	// @Sam
	//  I just happened to notice, there are some helper functions in chainlink-common/pkg/types/query that can make this a bit simpler. It looks like this should be equivalent to the expression I have above:
	// query.And(NewAddressFilter(addr), query.Or(NewEventSigFilter(event1), NewEventSigFilter(event2), NewEventSigFilter(event3)))
	...
	for _, log := range logs {
		...
		case ProductionConfigSet:
			...
			cp.cc.StoreConfig(ctx, event.ConfigDigest, event.Signers, event.F)

			isProduction := (cp.instanceType != InstanceTypeBlue) == event.IsFlipped
			if isProduction {
				latestLog = log
...
func (cp *configPoller) LatestConfig(ctx context.Context, changedInBlock uint64) (ocrtypes.ContractConfig, error) {
	cfg, _, err := cp.latestConfig(ctx, int64(changedInBlock), math.MaxInt64)
	if err != nil {
