...
func Test_ConfigPoller(t *testing.T) {
	...
	t.Run("Blue/Green config poller follow production and staging configs respectively initially", func(t *testing.T) {
		t.Run("LatestConfigDetails", func(t *testing.T) {
			...
			t.Run("with isFlipped=false", func(t *testing.T) {
				isFlipped := false
			...
			t.Run("with isFlipped=true (after PromoteStagingConfig)", func(t *testing.T) {
				isFlipped := true
			...
			t.Run("isFlipped=false again (another PromoteStagingConfig", func(t *testing.T) {
				isFlipped := false
...
func makeConfigSetLogData(t *testing.T,
	...
	data, err := event.Inputs.NonIndexed().Pack(previousConfigBlockNumber, configDigest, configCount, signers, offchainTransmitters, f, onchainConfig, offchainConfigVersion, offchainConfig, isFlipped)
	require.NoError(t, err)
	return data
}
